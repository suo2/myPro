...
public class GridListAdapter extends RecyclerView.Adapter<GridListAdapter.GridViewHolder> {
    ...
    public void onBindViewHolder(GridViewHolder holder, int position) {
        ...
        final HotCity data = mData.get(pos);
        if (data == null) {
            return;
        }
        if (data.isChecked()){
            holder.name.setBackgroundResource(R.drawable.cp_grid_item_select);
            holder.name.setTextColor(ContextCompat.getColor(mContext,R.color.text_color_ff9933));
        }else{
            holder.name.setBackgroundResource(R.drawable.cp_grid_item_bg);
            holder.name.setTextColor(ContextCompat.getColor(mContext,R.color.text_color_666666));
        }
        //设置item宽高
