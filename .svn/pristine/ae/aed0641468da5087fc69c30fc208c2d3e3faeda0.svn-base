    ...
    public static boolean checkPasword(String password) {
        ...
        for (int i = 0; i < password.length(); i++) {
            if (Character.isDigit(password.charAt(i))) {   //用char包装类中的判断数字的方法判断每一个字符
                isDigit = true;
            } else if (Character.isLowerCase(password.charAt(i))) {  //用char包装类中的判断字母的方法判断每一个字符
                isLowerCase = true;
            } else if (Character.isUpperCase(password.charAt(i))) {
                isUpperCase = true;
            }
        }
        boolean isRight = isDigit && (isLowerCase || isUpperCase) && isPassword(password);
        return isRight;
    }
